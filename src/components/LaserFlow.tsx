...
function colorToRGB(color: string) {
  ...
  try {
    parsed.setStyle(resolved || '#ffffff');
    return { r: parsed.r, g: parsed.g, b: parsed.b };
  } catch {
    return hexToRGB(resolved || '#ffffff');
  }
}
...
export const LaserFlow: React.FC<Props> = ({
  ...
  const applyThemeColor = useCallback(() => {
    ...
    if (!uniforms) return;
    const { r, g, b } = colorToRGB(color || '#FFFFFF');
    uniforms.uColor.value.set(r, g, b);
  }, [color]);

  useEffect(() => {
    applyColorRef.current = applyThemeColor;
    applyThemeColor();
  }, [applyThemeColor]);

  useEffect(() => {
    if (!color?.trim().startsWith('var(')) return;
    let raf = 0;
    let timer: ReturnType<typeof setTimeout> | null = null;
    let last = '';
    const tick = () => {
      const next = resolveThemeColor(color || '');
      if (next && next !== last) {
        last = next;
